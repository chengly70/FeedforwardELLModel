function [presy_Frq_tuned,J_di,W_di]=ftuning_wedge(qPreF,ThresE,Ne,Nei)
%determine the strength of I_aff and connect matrices (_ee,_ie) based on (qPreF,ThresE) relationship
%Based on distance wedge of 2 lines; slope: m_max->corr=1, -m_max->corr=-1, missing many (q,th) in upper(Quad1/2) and lower(Quad3/4)
...
eff_Corr = -1.6*(frq-fr_min)./(fr_max-fr_min) + .8; %len_frqv x 1
...
eps_wth=0.4; %cells within this band driven/connect. max
rnd_perm=randperm(len_frqv);
for k=1:len_frqv
    j=rnd_perm(k); %randomly go through all len_frqv frequencies
    %check to see if in wedge; if so, make connection prob=1
    InWedge = ( (ThresE-sing_pt(2) < (eff_Slp(j)+eps_wth)*(qPreF-sing_pt(1))) &...
        (ThresE-sing_pt(2) > (eff_Slp(j)-eps_wth)*(qPreF-sing_pt(1))) ) | ...
        ( (ThresE-sing_pt(2) > (eff_Slp(j)+eps_wth)*(qPreF-sing_pt(1))) &...
        (ThresE-sing_pt(2) < (eff_Slp(j)-eps_wth)*(qPreF-sing_pt(1))) ); %Ne x 1 logical
    
    dist_vr=dist_line(:,j); %distance to center of wedge (center line)
    
    prob_conn=exp(-(dist_vr)./.01); %Ne x 1, prob of connection
    prob_conn(InWedge)=1; %set probConn in wedge to 1
